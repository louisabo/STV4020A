...
# Den første delen av dette oppgavesettet begynner med 
# "tilrettelegging" av data, også kommer oppgavene helt på slutt
...
# redusere1, tillit, og rik1? 

mean(data$redusere1, na.rm = T) # 2.80629

mean(data$tillit, na.rm = T) # 5.280401

mean(data$rik1, na.rm = T) # 1.441744
     
### Oppgave 2:Lag en regresjonsanalyse med 
# rik1 som Y og redusere1 som X 

mod <- lm(rik1 ~ redusere1, data = data)

summary(mod)

### Oppgave 3: Vis resulatene i fin tabell (stargazer)
# Klarer du å tyde resulatene? 

library(stargazer)
stargazer(mod, type = "text", align = T)

### Oppgave 4: Lag en regresjonsanalyse med med 
# rik1 som Y og redusere1 og tillit som X 

mod1 <- lm(rik1 ~ redusere1 + tillit, data = data)

### Oppgave 5: Vis resulatene i en fin tabell (stargazer)

stargazer(mod1, type = "text", align = T)

### Oppgave 6: Fremstill resultatene fra den første regresjonen 
# linært vha ggplot 

ggplot(data, aes(x = redusere1, y = rik1)) + 
  geom_smooth(method = "lm")

### Oppgave 7: Fremstill resultatene fra den andre regresjonsen visuelt
# Her åpner jeg for alle mulige løsningsforslag
...
# OPPGAVE 8 
# Sentrer variablene redusere1 og tillit 
# Kjør en regresjonsanalyse med sentrert tillit som y 
# og sentrert redusere1 som x 
# Forstår du outputen? 

# Sentrering av redusere1
data$redusere1_sentrert <- scale(data$redusere1, center = T, scale = F)

# Sentrering av tillit
data$tillit_sentrert <- scale(data$tillit, center = T, scale = F)

# Lager OLS 

modell6 <- lm(tillit_sentrert ~ redusere1_sentrert, data = data)
